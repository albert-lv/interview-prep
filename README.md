...
[![Progress](https://img.shields.io/badge/进度-Week%2016%20Day%20107%20🎉-blue)](./memory/interview-prep.md)
[![Daily Update](https://img.shields.io/badge/🔥%20每日更新-已连续%20107%20天-success)](https://github.com/albert-lv/interview-prep/commits/main)
...
| 特性 | 说明 |
|---|---|
| 🔄 **每日双更** | 早上 Agent 工具技巧，晚上算法 + 面试考点，**已经连续更新 107 天** |
| 📅 **6 周系统计划** | 不是零散刷题，按主题递进（DP → 数据结构 → 网络 → 系统设计） |
| 🎤 **面试导向** | 每道题带「面试官会怎么问」+「一句话速答」 |
| ✅ **可运行代码** | 不是伪代码，是能直接 `gcc` 或 `go run` 的 |

### 今日更新（Day 107 · 2026-09-19 · Week 16 收官日 🎉）
- 🔥 [多数元素 + Week 16 分布式系统核心协议综合复习](week16/2026-09-19-week16-review-majority-voting.md) — Boyer-Moore 投票 O(n)/O(1) 模板收官（"少数派经不起消耗战"，和 Raft 选举同一个直觉）；Week 16 六大协议一张总表：Raft（强 Leader 三驾马车）/ Paxos（两阶段理论标杆）/ Gossip（反熵+谣传，O(log n) 轮收敛）/ 分布式锁（打分表六要素 + Redis 锁四代演进）/ CRDT（三定律 + G-Counter·OR-Set）/ OT（transform + CCI 模型）；三条主线串全周：CAP 选位置 → 共识造单一事实源 vs AP 三路线解决冲突 → 锁是一致性在具体场景的投影；10 道高频连环问通关速答 + 一段式收尾话术；Day 107 自检清单（Week 16 完结撒花，明天开启 Week 17）
- 🎤 面试技巧：六协议速查总表、多数派交集安全论证一句话版、LWW 三坑与 HLC 改进复述模板、CRDT 三定律默写卡、"讲讲分布式一致性"黄金收尾框架
...
当前进度：**Week 16 / Day 107**（Week 16 主题：分布式系统核心协议 🔗 — **已完结 🎉**，Day 107 综合复习收官，Week 17 即将开启）

**更新记录**：已连续更新 **107** 天，每日 20:42 自动推送。
